...
let battleLog = firestore.collection("battleLog");
...
idList.get().then( snapshot =>{
  ...
  snapshot.forEach((doc) =>{
    console.log(doc.id);
    trioModeList.forEach((mode) => {
      brawlerNameList.forEach((brawlerName) => {
        let numVictory = 0;
        let numDefeat = 0;
        let numDraw = 0;
        let totalChangedTropies = 0;

        battleLog
          .where("tag", "==", doc.id)
          .where("brawler_name", "==", brawlerName)
          .where("mode", "==", mode)
          .where("type", "==", "ranked")
          .where("isPowerPlay", "==", false)
          .get()
          .then((snapshot) => {
            if (snapshot.empty) {
              console.log("No matching documents.");
            ...
            snapshot.forEach(function (doc) {
              ...
              console.log(data);
              totalChangedTropies += data.trophyChange;

              //console.log(data.trophyChanges
            });
            console.log(numVictory, numDefeat, numDraw, totalChangedTropies);
            idList
              .doc(doc.id)
              .collection(mode)
              .doc("2020-04 "+brawlerName)
              .update({
                brawlerName : brawlerName,
                time : "2020-04",
                numVictory: numVictory,
                numDefeat: numDefeat,
                numDraw: numDraw,
                numRanked: numVictory + numDefeat + numDraw,
                changedTropies: totalChangedTropies,
              })
              .then(function () {
                console.log(`${doc.id} ${mode} successfully updated`);
              });
          });
      });
    });
